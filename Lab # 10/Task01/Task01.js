...
function click(ev, gl, canvas, program, isMouseDown) {
	...
	{
		rx = Math.sqrt(Math.pow(x - x1, 2.0) + Math.pow(y - y1, 2.0));
		ry = rx / 2;

		console.log("Radius X: " + rx);
		console.log("Radius Y: " + ry);

		calculateEllipsePoints(gl, program);
	}
}
...
function calculateEllipsePoints(gl, program)
{
	//Draw mid point
	//convertToWebGLCoords(x1, y1);
	...
	// For Region # 02
	var p2 = Math.pow(ry, 2) * Math.pow((x + 1/2), 2) + Math.pow(rx, 2) * Math.pow((y - 1), 2) - Math.pow(rx, 2) * Math.pow(ry, 2);
